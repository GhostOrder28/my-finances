...
These are the relevant settings when successfully running vue-cli and node, each on a different port, the only exception is on android, on android for any call I make from the cliente to the server I get a `net::ERR_CONNECTION_REFUSED`:
...
These are the relevant settings when running only the node server, which serves the static files generated by vue-cli, with this setting my app runs even on android just fine:
