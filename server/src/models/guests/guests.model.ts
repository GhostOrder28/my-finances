...
import { DateItem } from "../../types/guest.types.js";
import { Item } from "../../types/sale.types.js";
...
function getRandomItem (arr: string[] | DateItem[] | number[]) {
  const randomIdx = Math.floor(Math.random() * (arr.length - 1));
  return arr[randomIdx]
};

function getRandomLorem (units: ILoremIpsumParams['units'], count: number) {
...
function getItems () {
  return [
    { 
      ...
      pricePerUnit: getRandomItem(pricePerUnitPool) as number
    },
    { 
      name: getRandomLorem('words', 2),
      quantity: getRandomNumber(3) + 1,
...
function getPayments (paidAmount: number, numberOfPayments: number, paymentDates: string[]) {
...
function generateSale () {
  ...
  const selectedPaymentDates = getRandomItem(datePool) as DateItem;
...
async function signupGuest (body: SignupData) {
...
async function postGuestClient (userId: string, clientName: string) {
